...
## オブジェクト指向
Rust には class がないので `struct` と `impl` ブロックを使用して実現する。\
`struct` と `impl` の名前は同じにする必要がある。
...
`add`, `remove`, `avarage` が公開メソッド、`update_avarage` が非公開メソッドになる。\
構造体も `AveragedCollection`は公開しているが、 `list`, `avarage` の各フィールドは非公開となっているので、\
外部コードはメソッド経由でしかアクセスできない。（カプセル化）

### 継承
Rustには class と同様に継承という機能はない。\
コードの再利用に関してはトレイトを使用することで実現が可能。
多様性は `components` を使ったダックタイピングで実現が可能。
